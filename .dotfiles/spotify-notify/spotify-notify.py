...
class SpotifyNotify():
    ...
    def executeCommand(self, key):
        ...
        self.cmd = self.spotifyservice.get_dbus_method(
            key, 'org.mpris.MediaPlayer2.Player')
        self.cmd()
    ...
    def retrieveCoverImage(self, trackInfo):
        if 'arturl' in trackInfo:
            self.debug.out(
                "Simply retrieving image from {0}".format(trackInfo['arturl']))
    ...
    def startSpotify(Debug):
        ...
        if len(procs):
            Debug.out(
                "Spotify process found as: {0}".format(" ".join(procs[0])))
            return

        Debug.out("Starting new Spotify now.")

        FNULL = open('/dev/null', 'w')
        spid = subprocess.Popen([ident], stdout=FNULL, stderr=FNULL).pid
        if spid:
    ...
    def preventDuplicate(Debug):
        ...
        Debug.out("Looking for other processes named: "
                  "{0}".format(process).strip())

        if process == 'python':
            if not sys.argv[0]:
                Debug.out("Process started using python, "
                          "cannot determine script name.")
                return

            search = 'ps ax | grep "python {0}" | grep -v grep'.format(
                sys.argv[0])

        for line in SpotifyNotify.checkForProcess(search, Debug):
            if int(line[0]) != mypid:
                print("This program was already running.")
                Debug.out("I am a duplicate. I shall end myself. "
                          "({0})".format(" ".join(line)))
                exit(0)
    ...
    def checkForProcess(proc, Debug):
        ...
        sub = subprocess.Popen(
            proc, shell=True, stdout=subprocess.PIPE).stdout
        for line in sub:
            fields = line.split()
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action='store_true',
        default=False,
        help='Spotify will not be opened if it is not running and '
    ...
    if SPOTIFY_PROCESS_NAME:
        ...
        sub = subprocess.Popen(
            'which spotify', shell=True, stdout=subprocess.PIPE).stdout
        for line in sub:
    ...
    if not options.skipSpotify:
        gobject.timeout_add(
            SPOTIFY_CLOSED_CHECK, SN.checkForClosedSpotify, SN, Debug)
